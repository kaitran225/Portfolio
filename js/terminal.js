...
Terminal.Events = function(inputElement, OutputElement) {
    ...
    var body  = document.getElementById('cmdline');
    var pointer = document.getElementById('prompt').innerText;
    // Input Keypress
    input.onkeydown = function(event) {
        if (event.which == 13 || event.keyCode == 13) {
            ...
            // Get Command
            output.write(pointer + input.value)
...
Terminal.Output = function(element) {
    ...
    this.error = function(content) {
        var fromContent = outputElement.innerHTML;
        fromContent += '<div class="cmd-output-error">';
        fromContent += content;
        fromContent += '</div>';
        outputElement.innerHTML = fromContent;
        return this;
    };
    this.space = function(content) {
        var fromContent = outputElement.innerHTML;
        fromContent += '<div class="cmd-output">';
        fromContent += content;
        fromContent += '</div>';
...
Command.Notfound = {
    getFsCallback: function(input, output) {
        return output.error('\''+ input + '\' is not recognized as an internal or external command\, operable program or cybria file');
    }
};
